...
const _RATES = {
  ...
  opus:   { in: 15.0, out: 75.00, cc: 18.75, cr: 1.50  },
};
function _tokenCost(model, u) {
  const m = String(model || '').toLowerCase();
  const r = m.includes('haiku') ? _RATES.haiku : m.includes('opus') ? _RATES.opus : _RATES.sonnet;
  return (
    (u.input_tokens || 0) * r.in / 1e6 +
    (u.output_tokens || 0) * r.out / 1e6 +
    (u.cache_creation_input_tokens || 0) * r.cc / 1e6 +
    (u.cache_read_input_tokens || 0) * r.cr / 1e6
  );
}

// Usage monitor — config + monthly cost from JSONL files
function getUsageMonitor() {
  const home = os.homedir();
  const configFile = path.join(home, '.claude', 'usage-monitor.json');
  const cacheFile  = path.join(home, '.claude', 'metrics', 'usage-cache.json');

  let billingResetDay = 1;
  let monthlyLimitUSD = 100;
  let sessionLimitUSD = 8;
  try {
    if (fs.existsSync(configFile)) {
      const cfg = JSON.parse(fs.readFileSync(configFile, 'utf-8'));
      const d = parseInt(cfg.billingResetDay, 10);
      if (d >= 1 && d <= 28) billingResetDay = d;
      const l = parseFloat(cfg.monthlyLimitUSD);
      if (l > 0) monthlyLimitUSD = l;
      const s = parseFloat(cfg.sessionLimitUSD);
      if (s > 0) sessionLimitUSD = s;
    }
  } catch { /* use defaults */ }

  const now = new Date();
  let periodStart = new Date(now.getFullYear(), now.getMonth(), billingResetDay);
  if (periodStart > now) periodStart = new Date(now.getFullYear(), now.getMonth() - 1, billingResetDay);
  const periodStartMs = periodStart.getTime();

  let nextReset = new Date(now.getFullYear(), now.getMonth(), billingResetDay);
  if (nextReset <= now) nextReset = new Date(now.getFullYear(), now.getMonth() + 1, billingResetDay);
  const msPerDay = 1000 * 60 * 60 * 24;
  const daysUntilReset = Math.max(1, Math.ceil((nextReset - now) / msPerDay));
  const resetLabel = nextReset.toLocaleString('en-US', { month: 'short' }) + ' ' + nextReset.getDate();
  ...
  try {
    if (fs.existsSync(cacheFile)) {
      ...
      if (latestMtime <= (cache.lastMtime || 0) && cache.periodStart === periodStart.toISOString()) {
        return { periodCost: cache.periodCost, daysUntilReset, resetLabel, monthlyLimitUSD, sessionLimitUSD };
      }
    }
  } catch { /* recompute */ }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (entry.timestamp && new Date(entry.timestamp).getTime() < periodStartMs) continue;
          periodCost += _tokenCost(msg.model || entry.model || '', msg.usage);
        } catch { /* skip */ }
      }
    } catch { /* skip file */ }
  }
  ...
  try {
    ...
    fs.writeFileSync(cacheFile, JSON.stringify({ periodCost, lastMtime: latestMtime, periodStart: periodStart.toISOString() }));
  } catch { /* non-fatal */ }

  return { periodCost, daysUntilReset, resetLabel, monthlyLimitUSD, sessionLimitUSD };
}

// Current session usage — tracks tokens in most-recent JSONL + approximates Pro reset (~5h window)
function getSessionUsage(sessionLimitUSD) {
  const home = os.homedir();
  const projectsDir = path.join(home, '.claude', 'projects');
  let latestFile = null;
  let latestMtime = 0;
  ...
  if (!latestFile) return { cost: 0, pct: 0, resetLabel: '–', resetSoon: false };

  let sessionCost = 0;
  let sessionStartMs = latestMtime;
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (entry.timestamp) {
          ...
          if (ts < sessionStartMs) sessionStartMs = ts;
        }
        const msg = entry.message || {};
        if (msg.usage) sessionCost += _tokenCost(msg.model || entry.model || '', msg.usage);
      } catch { /* skip */ }
    }
  } catch { /* skip */ }

  const SESSION_WINDOW_MS = 5 * 60 * 60 * 1000;
  const remaining = Math.max(0, SESSION_WINDOW_MS - (Date.now() - sessionStartMs));
  const resetMins = Math.ceil(remaining / 60000);
  const resetLabel = resetMins <= 0 ? 'soon'
    : resetMins < 60 ? resetMins + 'm'
    : Math.floor(resetMins / 60) + 'h ' + (resetMins % 60) + 'm';

  const pct = Math.min(100, Math.round((sessionCost / (sessionLimitUSD || 8)) * 100));
  return { cost: sessionCost, pct, resetLabel, resetSoon: resetMins < 30 };
}
...
function generateStatusline() {
  ...
  const sessUsage = getSessionUsage(usage.sessionLimitUSD);
  ...
  // LINE 1: Plan usage bar \u2014 uses the REAL Claude Pro rate limit data piped by
  // Claude Code via stdin (data.rate_limits.five_hour.used_percentage).
  ...
  // DDD Domains + Blitz target line removed (slim dashboard)
  ...
  // Architecture + Security line removed (slim dashboard)
  ...
  // MCP \u2014 kept last
  lines.push(c.brightCyan + '\uD83D\uDD0C ' + c.reset + integStr);
...
// Get Claude.ai Pro plan rate limits from Claude Code stdin.
// These are the exact same numbers shown in Claude settings → Plan usage.
// five_hour = "Aktuelle Sitzung" bar; seven_day = "Wöchentliche Limits".
// Only present after the first API response; returns null when not yet available.
